 ...
 * @return : return's head node after insertion of elements, wether at
 * beginning/end or k.
 */
static auto insertKthPosition(Node *head, int k, int ele) -> Node * {
    ...
    if (mover->next != nullptr) {
        // this is to connect the node of newNode right side, if only its not a
        // nullptr, cause if its a nullptr then newNode's by default have
        // nullptr.
 ...
 */
static auto deleteKthNode(Node *head, int k) -> Node * {
    ...
    if (k == sizeLL - 1) {
        Node *mover = head;
        while (mover->next != nullptr) {  // Move to the last node.
            mover = mover->next;
        }
    ...
    if (mover->next != nullptr) {  // Check to avoid dereferencing nullptr.
 ...
 * @param : Node* head -> This is the head Node which we will use to traverse in
 * the LL.
 *
 * @return : Returns nothing, as it is use to free the memory.
 */
static void freeLL(Node *head) {
    while (head != nullptr) {
        Node *temp = head;
        head = head->next;
        free(temp);
...
auto main() -> int {
    ...
    while (running) {
        ...
        switch (choice) {
            ...
            case 4:  // Get size
                cout << "Current size of linked list: " << sizeLL << '\n';
                break;
